...
- Ограничения Like/Match/Report защищают от дубликатов, но для существующих инсталляций становятся гарантией БД только после применения Alembic-миграции.
- `report_count` является денормализованным счётчиком; при высокой конкурентности нужен атомарный `UPDATE ... RETURNING` или агрегирующая задача.
