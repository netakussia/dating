...
- Фото принимается как Telegram-photo и после сохранения проверяется PhotoModerationService;
	NSFW/критические сигналы лица скрывают профиль и создают moderation case, а ошибка ML-провайдера
	переводит проверку в YELLOW, оставляет профиль доступным и создаёт кейс для ручной проверки.
