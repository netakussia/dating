...
async def toggle(callback: CallbackQuery, session: AsyncSession) -> None:
    ...
    if p: p.is_visible = not p.is_visible; await callback.message.edit_reply_markup(reply_markup=profile_keyboard(p.is_visible)); await callback.answer("Видимость изменена")
