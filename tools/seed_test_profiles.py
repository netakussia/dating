...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--photo-file-id", type=str, default=None, help="Optional Telegram file_id to use for all photos (useful for real-file testing)")
    parser.add_argument("--target-gender", type=str, choices=["MALE", "FEMALE", "ALL"], default=None, help="Optional target gender for created profiles")
...
async def _seed(count: int, photo_file_id: str | None, target_gender: str | None = None) -> List[int]:
    ...
    print(f"Created {len(created_ids)} test users: {created_ids[:5]}{'...' if len(created_ids)>5 else ''}")

    # Now trigger rebuild_queue for each created user so Redis queues are filled
    # We'll create a short-living session per user to run the rebuild which relies on services.recommendation.RecommendationService
