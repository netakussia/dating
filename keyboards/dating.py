...
def dating_keyboard(user_id: int):
    ...
    kb.button(text="❤️", callback_data=f"like:{user_id}")
    kb.button(text="💌", callback_data=f"comment:{user_id}")
    kb.button(text="👎", callback_data=f"skip:{user_id}")
    kb.button(text="⚠️", callback_data=f"report:{user_id}")
    kb.button(text="🚫", callback_data=f"block:{user_id}")
    kb.adjust(4, 1)
    return kb.as_markup()
